...
STEPS = [
    {
        "name": "csi300_global_local_stable_summary",
        "workflow_module": "next_step.hierarchical_state_field.experiment.configs.ref_all_hsf_csi300",
        "market_state_path": "artifacts/market_state/daily_market_field_ref_all_SH000300.pkl",
        "label": "global_local_stable_summary_ref_all_SH000300",
        "expected_backtest_benchmark": "SH000300",
    },
    {
        "name": "csi800_global_local_stable_summary",
        "workflow_module": "next_step.hierarchical_state_field.experiment.configs.ref_all_hsf_csi800",
        "market_state_path": "artifacts/market_state/daily_market_field_ref_all_SH000906.pkl",
        "label": "global_local_stable_summary_ref_all_SH000906",
        "expected_backtest_benchmark": "SH000906",
    },
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the first ref-all HSF target round: global_local_stable_summary only.",
    )
    parser.add_argument(
        "--python",
        dest="python_exe",
        default=_default_python(),
        help="Python executable for the qlib environment.",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
        help="Seed override passed to the variant script. Defaults to the dedicated configs' seed.",
    )
    parser.add_argument(
        "--run-tag",
        default=dt.datetime.now().strftime("ref_all_hsf_first_round_%Y%m%d_%H%M%S"),
        help="Run directory name under experiment/runs and experiment_suffix in MLflow.",
    )
    parser.add_argument(
        "--only",
        choices=["all", "csi300", "csi800"],
        default="all",
        help="Run both configs or a single universe.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Only write commands/status files; do not launch training.",
    )
...
def main() -> int:
    ...
    selected = [
        step
        for step in STEPS
        if args.only == "all" or step["name"].startswith(args.only)
    ]
    run_dir = RUNS_DIR / args.run_tag
    run_dir.mkdir(parents=True, exist_ok=True)

    commands = {
        step["name"]: _build_command(step, args.python_exe, args.run_tag, args.seed)
    ...
    status: dict[str, Any] = {
        ...
        "policy": "first_round_global_local_stable_summary_only",
        ...
        "steps": {
            step["name"]: {
                ...
                "label": step["label"],
                "expected_backtest_benchmark": step["expected_backtest_benchmark"],
    ...
    _write_text(
        run_dir / "README.md",
        "# Ref-All HSF First Round\n\n"
        "- Scope: global_local_stable_summary only.\n"
        "- Train segment: 2008-01-30 to 2020-03-31.\n"
        "- Eval/test/backtest segment: 2020-07-01 to 2022-12-31.\n"
        "- Value embedding: feature_tokenizer.\n"
        "- Pooling mode: full (conditioned query + adaptive alpha).\n"
        "- Shared observation: artifacts/market_state/daily_market_observation_ref_all_pti_v1.pkl.\n"
        "- CSI300 field: artifacts/market_state/daily_market_field_ref_all_SH000300.pkl.\n"
        "- CSI800 field: artifacts/market_state/daily_market_field_ref_all_SH000906.pkl.\n"
        "- CSI800 batch policy: train batch_size=400, eval_batch_size=800.\n",
    )
