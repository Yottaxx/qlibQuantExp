...
STEP = {
    "name": "resgru_mean_legacy_summary_stockrel",
    "label": "csi300_tri_scope_resgru_mean_legacy_summary_stockrel",
    "workflow_module": (
        "next_step.hierarchical_state_field.experiment.configs."
        "ref_all_hsf_csi300_tri_scope_resgru_mean_legacy_summary_stockrel"
    ),
    "intent": "v5 probe: legacy layer-summary concat plus stock-level inner cross-stock relation.",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CSI300 resGRU mean legacy-summary stock-relation probe.")
    parser.add_argument("--python", dest="python_exe", default=_default_python())
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument(
        "--run-tag",
        default=dt.datetime.now().strftime("hsf_resgru_mean_legacy_summary_stockrel_%Y%m%d_%H%M%S"),
    )
...
def main() -> int:
    ...
    status: dict[str, Any] = {
        ...
        "skip_visuals": not bool(args.with_visuals),
        "policy": "legacy_summary_stock_relation_probe",
    ...
    _write_text(
        run_dir / "README.md",
        "# ResGRU Mean Legacy Summary Stock-Relation Probe\n\n"
        "- Baseline: `ref_all_hsf_csi300_tri_scope_resgru_mean_bounded_inner`.\n"
        "- Deltas: factor gate 1.0/0.2, per-layer batch summary legacy concat, stock-level inner relation.\n"
