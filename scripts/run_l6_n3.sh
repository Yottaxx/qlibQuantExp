#!/usr/bin/env bash
# L-6 portfolio-IR aux — n=3 PAIRED ablation (card h-20260627-001).
# 3 seeds x {treatment lambda=0.08, control lambda=0}, readout OFF (bare tau0.5 backbone),
# 25ep from-scratch, FULL window, COST-ON backtest. work_flow.py untouched (env overrides only).
# Order: treatment-s42 FIRST (riskiest: exercises l_ir on full-window fp16, post MF-1 fix) for early de-risk.
set -u
...
# cost-ON + limit-threshold backtest so IR_wc / MaxDD_wc are realistic (defaults are frictions-OFF).
PORT='{"backtest":{"exchange_kwargs":{"open_cost":0.0005,"close_cost":0.0015,"min_cost":5,"limit_threshold":0.095}}}'

run () {
  local arm="$1" lam="$2" seed="$3"
  local setting="l6_${arm}_s${seed}"
...
run ir008 0.08 42   # treatment first (de-risk full-window fp16)
run ir000 0.0  42   # paired control
run ir008 0.08 43
run ir000 0.0  43
run ir008 0.08 44
run ir000 0.0  44
echo "=== $(date '+%F %T') ALL 6 RUNS DONE ==="
